...
var services = [{
  ...
  skip: function (query, result) {
    // some other service already calculated directions
    // or asking for alternate routes
    // or asking to include seasonally closed roads
    return result || query.alternate || query.seasonal;
  }
}, {
  name: 'googlews',
  ...
  skip: function (query, result) {
    // some other service already calculated directions
    // or is in the past
    return result || !util.isFuture(query.begin);
  }
}, {
  name: 'mapquest',
