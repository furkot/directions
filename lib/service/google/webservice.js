...
function toCoordinates(pt) {
  ...
  if (isInChina) {
    pt = eviltransform.wgs2gcj(pt[1], pt[0]);
    return [pt.lat, pt.lng].join(',');
  }
  return [pt[1], pt[0]].join(',');
}

function transform(pt) {
  pt = eviltransform.gcj2wsg(pt[1], pt[0]);
  return [pt.lat, pt.lng].join(',');
}

function decode(path, isInChina) {
...
function extractSegment(directions, step) {
  directions.segments.push({
      ...
      path: step.polyline && decode(step.polyline.points, directions.query.isInChina),
...
function init(options) {
  ...
  function prepareRequest(query) {
    var req = {
      ...
      origin: toCoordinates.call(query, query.points[0]),
      destination: toCoordinates.call(query, util.last(query.points)),
    ...
    if (query.points.length > 2) {
      req.waypoints = query.points.slice(1, query.points.length - 1).map(toCoordinates, query).join('|');
    }
    ...
    if (response && response.status === gmStatus.OK && response.routes && response.routes[0]) {
      ...
      if (rtLeg) {
        ...
        if (route.overview_polyline && route.overview_polyline.points && query.path === pathType.coarse) {
          ...
          partition(decode(route.overview_polyline.points, query.isInChina), directions.routes, directions.segments);
        }
      }
    }
    return directions;
