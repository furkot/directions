...
function extractSegment(directions, step) {
  directions.segments.push({
      ...
      path: step.polyline && step.polyline.points && polyline.decode(step.polyline.points).map(reverse),
  ...
  if (response && response.status === gmStatus.OK && response.routes && response.routes[0]) {
    ...
    if (rtLeg) {
      ...
      if (route.overview_polyline && route.overview_polyline.points) {
        directions.routes[0].path = polyline.decode(route.overview_polyline.points).map(reverse);
...
function init(options) {
  ...
  options = _.defaults(options || {}, {
    ...
    url: '/service/googlemaps/maps/api/directions/json',
    status: getStatus,
